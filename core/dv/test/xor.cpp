...
TEST(Arithmetic, Xor) {
  for (auto t : xor_op::arithmetic_tests) {
    ...
    tc::test_rr_op(instruction_memory, &riscv::xor_op, t.result, t.value1,
                   t.value2, 32);
...
TEST(Arithmetic, XorSrc1EqDest) {
  for (auto t : xor_op::arithmetic_tests) {
    VDutCore *core = new VDutCore("Core");
    Verilated::traceEverOn(true);
    VerilatedVcdC *tfp = new VerilatedVcdC;
    core->trace(tfp, 99);
    tfp->open("XorSrc1EqDest.vcd");

    std::vector<uint32_t> instruction_memory;
    std::vector<uint32_t> data_memory;
    tc::test_rr_src1_eq_dest(instruction_memory, &riscv::xor_op, t.result,
...
TEST(Arithmetic, XorSrc2EqDest) {
  for (auto t : xor_op::arithmetic_tests) {
    VDutCore *core = new VDutCore("Core");
    Verilated::traceEverOn(true);
    VerilatedVcdC *tfp = new VerilatedVcdC;
    core->trace(tfp, 99);
    tfp->open("XorSrc2EqDest.vcd");

    std::vector<uint32_t> instruction_memory;
    std::vector<uint32_t> data_memory;
    tc::test_rr_src2_eq_dest(instruction_memory, &riscv::xor_op, t.result,
                             t.value1, t.value2, 32);
...
TEST(Arithmetic, XorSrc12EqDest) {
  for (auto t : xor_op::same_source_tests) {
    VDutCore *core = new VDutCore("Core");
    Verilated::traceEverOn(true);
    VerilatedVcdC *tfp = new VerilatedVcdC;
    core->trace(tfp, 99);
    tfp->open("XorSrc12EqDest.vcd");

    std::vector<uint32_t> instruction_memory;
    std::vector<uint32_t> data_memory;
    tc::test_rr_src12_eq_dest(instruction_memory, &riscv::xor_op, t.result,
                              t.value, 32);
...
TEST(Arithmetic, XorDestBypass) {
  for (auto t : xor_op::dest_bypass_tests) {
    VDutCore *core = new VDutCore("Core");
    Verilated::traceEverOn(true);
    VerilatedVcdC *tfp = new VerilatedVcdC;
    core->trace(tfp, 99);
    tfp->open("XorDestBypass.vcd");

    std::vector<uint32_t> instruction_memory;
    std::vector<uint32_t> data_memory;
    tc::test_rr_dest_bypass(instruction_memory, t.nop, &riscv::xor_op,
...
TEST(Arithmetic, XorSrc12BypassTest) {
  for (auto t : xor_op::src12_bypass_tests) {
    VDutCore *core = new VDutCore("Core");
    Verilated::traceEverOn(true);
    VerilatedVcdC *tfp = new VerilatedVcdC;
    core->trace(tfp, 99);
    tfp->open("XorSrc12Bypass.vcd");

    std::vector<uint32_t> instruction_memory;
    std::vector<uint32_t> data_memory;
    tc::test_rr_src12_bypass(instruction_memory, t.nop1, t.nop2, &riscv::xor_op,
